...
## Starten en inrichten van een kubernetes cluster
We gaan gebruik maken van een Kubernetes cluster. Ik heb dit op twee manieren geprobeerd dus volg een van de van twee opties.  

### Optie A: starten Kubernetes cluster in colima (MacoOS)
...
Een standaard cluster in colima (k3s) bevat geen UI maar dat is tijdens het werk wel prettig om te hebben. HIervoor kun je headlamp gebruiken. Als dit nog niet geinstalleerd kun je het installeren met het volgende commando:  
...
### Optie B: starten Kubernetes cluster in podman desktop met kind (Windows)
Installeer podman desktop en start podman machine met voldoende capaciteit. Ik heb gebruik gemaakt van 24GB memory en 12 CPUs. Installeer vervolgens via de UI een kind cluster en zorg dat deze in Settings -> Kubernetes -> Kubernetes Contexts verbonden is. 
  

## Inrichten van de infrastructuur
Voer de README uit in van de volgende subfolders om de infrastructuur in te richten:
